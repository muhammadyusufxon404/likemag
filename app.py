...
async def start(update: Update, context: CallbackContext):
    ...
    # Super admin uchun qo'shimcha tugma
...
# Super admin xabarlarini qabul qilish
...
async def run_bot():
    ...
    app_bot.add_handler(CommandHandler("start", start))
    app_bot.add_handler(CallbackQueryHandler(handle_callback))
    app_bot.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, super_admin_message_handler))
    app_bot.job_queue.run_daily(send_daily_report, time=dtime(hour=23, minute=59, tzinfo=pytz.timezone('Asia/Tashkent')))
    print("✅ Bot ishga tushdi.")
    await app_bot.run_polling()
